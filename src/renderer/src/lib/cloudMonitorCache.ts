...
// 重进会白屏并重新拉取一遍远端接口。这里按 sourceId 保存最近一次成功拉取的快照
// （模块级内存，随渲染进程存活）：
...
/** 读快照（无记录时 undefined） */
export function getCloudSnapshot(sourceId: string): CloudSnapshot | undefined {
  return snapshots.get(sourceId)
}

/** 局部更新快照（未提及的字段保持原值） */
export function patchCloudSnapshot(sourceId: string, patch: Partial<CloudSnapshot>): void {
  snapshots.set(sourceId, { ...(snapshots.get(sourceId) ?? EMPTY), ...patch })
}

/** 清除快照（登出时调用：换账号后不得残留旧账号数据） */
export function clearCloudSnapshot(sourceId: string): void {
