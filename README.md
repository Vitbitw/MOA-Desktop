...
- **固定聚合输出** — 第三方客户端无论发什么，网关始终返回一份融合后的最终答案；协作架构跟随 MoA 配置（或网关单独指定）
...
- **☁️ 云端用量监控** — 内建 Command Code / 小米 MiMo / DeepSeek 三类云账号源：应用内登录后拉取余额、额度窗口（5 小时 / 7 天 / 月度）、订阅套餐与到期时间；Command Code 支持后台自动采集与本地累计（不受服务端记录条数上限影响）

### 平台与体验

- **🏭 多厂商** — 内置 38 家厂商模板（OpenAI / Anthropic / Gemini / DeepSeek / 硅基流动 / 智谱 / Kimi / Groq / OpenRouter 等），亦支持自定义地址
...
网关设置（设置页）：启用开关、监听地址 / 端口、并发上限、API Key 鉴权、请求记录（全量 / 仅统计）、透明模式（extended 时在响应中附加子模型执行明细）。
